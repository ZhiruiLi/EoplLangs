...
-- | OptionalTypeName ::= ConcreteTypeName
--                    ::= ?
optionalTypeName :: Parser (Maybe Type)
optionalTypeName = try (fmap Just concreteTypeName)
               <|> (questionMark >> return Nothing)

-- | ConcreteTypeName ::= int
--                    ::= bool
--                    ::= (({ConcreteTypeName}*(,)) -> ConcreteTypeName)
concreteTypeName :: Parser Type
concreteTypeName = try procTypeName
               <|> basicTypeName
  where
    procTypeName = parens $ do
      paramTypes <- parens $ sepBy concreteTypeName comma
      arrow
      resType <- concreteTypeName
...
typedParam :: Parser (String, Maybe Type)
typedParam = do
  name <- identifier
  colon
  typ <- optionalTypeName
  return (name, typ)
