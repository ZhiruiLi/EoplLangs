...
reservedWords  =
  ...
  , "begin", "set", "setdynamic", "during", "pair", "left", "right"
  , "setleft", "setright"
  ]
...
-- | NewPairExpr ::= pair (Expression, Expression)
newPairExpr :: Parser Expression
newPairExpr = do
  keyWord "pair"
  pair <- expressionPair
  return $ uncurry NewPairExpr pair

-- | GetLeftExpr ::= left (Expression)
getLeftExpr :: Parser Expression
getLeftExpr = GetLeftExpr <$> (keyWord "left" >> parens expression)

-- | GetRightExpr ::= right (Expression)
getRightExpr :: Parser Expression
getRightExpr = GetRightExpr <$> (keyWord "right" >> parens expression)

-- | SetLeftExpr ::= setleft (Expression, Expression)
setLeftExpr :: Parser Expression
setLeftExpr = uncurry SetLeftExpr <$> (keyWord "setleft" >> expressionPair)

-- | SetRightExpr ::= setright (Expression, Expression)
setRightExpr :: Parser Expression
setRightExpr = uncurry SetRightExpr <$> (keyWord "setright" >> expressionPair)
