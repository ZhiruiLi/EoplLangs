module InferredLang.ParserSuite
( tests
) where
...
testCondExpr = TestList
  [ testEq "Parse if expression"
           (IfExpr (UnaryOpExpr IsZero (constNum 3))
                   (constNum 4)
                   (constNum 5))
...
testLetExpr = TestList
  ...
  , testEq "Parse let recursive expression"
         (LetRecExpr
           [(Just TypeInt, "double", [("xxx", Just TypeInt)], BinOpExpr Mul (constNum 2) (VarExpr "xxx"))]
...
testParseProc = TestList
  [ testEq "Parse proc expression"
           (ProcExpr [("x", Just TypeInt)] (VarExpr "x"))
           "proc (x: int) x"
  , testEq "Parse proc expression with multi parameters"
           (ProcExpr [("x", Just TypeInt), ("y", Just TypeBool)] (VarExpr "x"))
           "proc (x: int, y: bool) x"
  , testEq "Parse call expression"
           (CallExpr
             (ProcExpr
               [("f", Just $ TypeProc [TypeInt, TypeInt] (TypeProc [TypeBool] TypeBool))]
               (CallExpr (VarExpr "f") [CallExpr (VarExpr "f") [constNum 77]]))
             [ProcExpr [("x", Just TypeBool)] (BinOpExpr Sub (VarExpr "x") (constNum 11))])
